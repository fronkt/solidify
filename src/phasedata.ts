...
export const PHASE_TABLE_VERSION = "1.1.1";
...
export interface BinaryRow {
  ...
  source: string;
  /**
   * The row's citation as the phase diagram prints it (v8 U1c): the source and
   * the independent recomputation in one complete short line, keeping any
   * weakness the reader must see (disputed, figure-read, digitized, a field
   * taken from the recomputation). It replaces a 240-character cut of
   * `source`, which ended mid-sentence; `source` stays the audit record.
   * Presentation only: it changes no number, so it does not bump
   * PHASE_TABLE_VERSION.
   */
  cite: string;
}

/** ASCII phase and reaction spellings as a reader expects them: θ, β, γ, δ, α, η and → */
function typeset(s: string): string {
  return s.replace(/->/g, "→").replace(/\b(theta|alpha|beta|gamma|delta|eta)\b/g,
    w => ({ theta: "θ", alpha: "α", beta: "β", gamma: "γ", delta: "δ", eta: "η" })[w] ?? w);
}
...
export function shortPhase(second: string): string {
  ...
  // v8 U1c: the name reaches the screen, so "gamma-austenite" prints as
  // γ-austenite, the way the prose around it writes it. A name that
  // typesetting shrinks to ONE character (Cu–Sn's and Cu–Zn's bare "beta")
  // takes the word "phase", exactly as the R phase does above: "(Cu) + β"
  // under a diagram names less than "(Cu) + β phase", and PD-REGIME-EXACT
  // holds every phase name to two characters or more.
  const out = typeset(m[0]);
  return out.length <= 1 ? `${out} phase` : out;
}
...
export function reactionText(row: BinaryRow): string {
  return typeset(row.reaction.split("[")[0].trim().replace(/\s+/g, " "));
}
...
export const BINARY: Record<string, Record<string, BinaryRow>> = {
  al: {
    Cu: {
      ...
      cite: "Murray 1985, Int. Metals Rev. 30 · checked against MatCalc mc_al (pycalphad)",
    ...
    Si: {
      ...
      cite: "Murray & McAlister 1984, via Buehler Tech-Notes 5 · checked against MatCalc mc_al (pycalphad)",
    ...
    Mg: {
      ...
      cite: "Murray 1982, Bull. Alloy Phase Diagrams 3; C_inv spread 34–35.5 wt% · checked against MatCalc mc_al",
    ...
    Zn: {
      ...
      cite: "Murray 1983, via Chen & Chang 1993 (Calphad 17) · checked against MatCalc mc_al (pycalphad)",
    ...
    Fe: {
      ...
      cite: "Kattner & Burton 1993 (ASM); C_inv disputed at ±0.1 wt% · checked against MatCalc mc_al",
    ...
    Ti: {
      ...
      cite: "Schuster & Palm 2006, J. Phase Equilib. Diffus. 27 · checked against MatCalc mc_al (pycalphad)",
  ...
  fe: {
    C: {
      ...
      cite: "Okamoto (ASM evaluation); Thermo-Calc TCFE7 via Alves et al. 2018 · checked against MatCalc mc_fe",
    ...
    Mn: {
      ...
      cite: "Thermo-Calc TCFE7 via Alves et al. 2018, a single source (disputed) · checked against MatCalc mc_fe",
    ...
    Si: {
      ...
      cite: "C_inv: Cui & Jung 2017, via Edmund et al. 2022; T and C_SM read off a figure · no invariant in the recomputation",
    ...
    Ni: {
      ...
      cite: "Swartzendruber et al. 1991 (T, C_SM); C_inv from MatCalc mc_fe, no cited value found",
    ...
    Cr: {
      ...
      reaction: "",
      second: "",
      cite: "Sun et al. 2025, arXiv:2507.16627: no liquid invariant (isomorphous) · the recomputation agrees",
    ...
    Mo: {
      ...
      cite: "Sun et al. 2025; Rajkumar & Hari Kumar 2014; reaction type disputed · checked against MatCalc mc_fe",
  ...
  ni: {
    Nb: {
      ...
      cite: "Okamoto 1998, after Bolcavage & Kattner 1996 · checked against MatCalc mc_ni (pycalphad)",
    ...
    Ti: {
      ...
      cite: "Massalski-type diagram, compositions digitized (±0.5 wt%) · checked against MatCalc mc_ni",
    ...
    Al: {
      ...
      // a reader's name, as every other row writes it: "NIAL" was the CALPHAD
      // phase id and reached the screen as "(Ni) + NIAL" (v8 U1c review)
      second: "beta-NiAl (B2, CsCl-type; NIAL in CALPHAD notation)",
      cite: "Okamoto 2004, J. Phase Equilib. Diffus. 25; eutectic taken from the MatCalc mc_ni recomputation (disputed)",
    ...
    Cr: {
      ...
      cite: "calphad.com 2006 vector diagram; Nash (ASM) · checked against MatCalc mc_ni (pycalphad)",
    ...
    Mo: {
      ...
      cite: "Okamoto 1991, after Frisk 1990; compositions digitized · checked against MatCalc mc_ni",
    ...
    W: {
      ...
      cite: "Okamoto 1991, after Nagender Naidu et al. 1986; geometrically inconsistent, not drawn",
  ...
  mg: {
    Al: {
      ...
      cite: "Nie 2012, Metall. Mater. Trans. A 43 (T, C_SM); C_inv recalled from ASM · checked against MatCalc mc_al",
    ...
    Zn: {
      ...
      cite: "Ghosh et al. 2012, Calphad 36; relabeled a eutectic · checked against MatCalc mc_al (pycalphad)",
    ...
    Zr: {
      ...
      cite: "Schaum & Burnett 1952, J. Res. NBS 49 · checked against MatCalc mc_al (pycalphad)",
  ...
  cu: {
    Sn: {
      ...
      cite: "Saunders & Miodownik 1994 (ASM), via Li et al. 2013 · checked against the NIST solder database",
    ...
    Zn: {
      ...
      cite: "Miodownik 1994 (ASM), via Tang et al. 2022 · checked against MatCalc mc_al (pycalphad)",
    ...
    Ni: {
      ...
      reaction: "",
      second: "",
      cite: "Chakrabarti et al. 1991 (ASM): complete solid solubility (isomorphous) · the recomputation agrees",

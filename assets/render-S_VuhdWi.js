var T=Object.defineProperty;var B=(p,t,e)=>t in p?T(p,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):p[t]=e;var n=(p,t,e)=>B(p,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function e(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(i){if(i.ep)return;i.ep=!0;const s=e(i);fetch(i.href,s)}})();const d=4096,w=64,x=6,P=`
struct Params {
  n: u32,
  frame: u32,
  dx: f32,
...
@group(0) @binding(1) var state: texture_2d<f32>;
@group(0) @binding(2) var grain: texture_2d<u32>;
@group(0) @binding(3) var<storage, read> seeds: array<f32, ${w*x}>;
@group(0) @binding(4) var stateOut: texture_storage_2d<rgba32float, write>;
@group(0) @binding(5) var grainOut: texture_storage_2d<r32uint, write>;
...
`,_=["MELT","ORIENT","ETCH","FIELD","RINGS","THERM","SEM","NEON","XRAY","CURV"],S=`
struct RParams {
  view: u32,
  n: u32,
  canvasW: f32,
...
`,z={dx:.03,dt:12e-5,epsBar:.01,delta:.04,aniMode:4,tau:3e-4,alpha:.9,gamma:10,latent:1.6,noiseAmp:.01,tFar:0,coolRate:0,heatIn:0,scen:0,gradG:.08,pullV:1.5,weldX:0,weldY:0,weldPow:700,weldSig:4,alloyOn:0,c0:.3,mLiq:.45,kPart:.2,dSol:.8,twinProb:0,meltGlow:1},E=1;class C{constructor(t,e){n(this,"device");n(this,"n");n(this,"params");n(this,"frame",0);n(this,"simTime",0);n(this,"dir",0);n(this,"nextId",1);n(this,"frontX",0);n(this,"stateTex",[]);n(this,"grainTex",[]);n(this,"fluxTex");n(this,"paramBuf");n(this,"theta0Buf");n(this,"twinCtrBuf");n(this,"seedBuf");n(this,"statsBuf");n(this,"statsStaging");n(this,"theta0CPU",new Float32Array(d));n(this,"fluxPipe");n(this,"updatePipe");n(this,"stampPipe");n(this,"statsPipe");n(this,"fluxBG",[]);n(this,"updateBG",[]);n(this,"stampBG",[]);n(this,"statsBG",[]);n(this,"pendingSeeds",[]);n(this,"pendingQuench",0);n(this,"statsInFlight",!1);n(this,"paramData",new ArrayBuffer(128));n(this,"inFlight",0);this.device=t,this.n=e,this.params={...z},this.build()}get busy(){return this.inFlight>=2}get theta0Buffer(){return this.theta0Buf}stateTexture(t){return this.stateTex[t]}grainTexture(t){return this.grainTex[t]}build(){const t=this.device,e=this.n,a=r=>({size:[e,e],format:r,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST});this.stateTex=[t.createTexture(a("rgba32float")),t.createTexture(a("rgba32float"))],this.grainTex=[t.createTexture(a("r32uint")),t.createTexture(a("r32uint"))],this.fluxTex=t.createTexture(a("rgba32float")),this.paramBuf=t.createBuffer({size:128,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.theta0Buf=t.createBuffer({size:d*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.twinCtrBuf=t.createBuffer({size:4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.seedBuf=t.createBuffer({size:w*x*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});const i=(4+d)*4;this.statsBuf=t.createBuffer({size:i,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.statsStaging=t.createBuffer({size:i,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});const s=r=>t.createComputePipeline({layout:"auto",compute:{module:t.createShaderModule({code:r}),entryPoint:"main"}});this.fluxPipe=s(R),this.updatePipe=s(M),this.stampPipe=s(G),this.statsPipe=s(L);for(const r of[0,1]){const o=this.stateTex[r].createView(),c=this.grainTex[r].createView(),l=this.stateTex[1-r].createView(),u=this.grainTex[1-r].createView();this.fluxBG[r]=t.createBindGroup({layout:this.fluxPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.theta0Buf}},{binding:4,resource:this.fluxTex.createView()}]}),this.updateBG[r]=t.createBindGroup({layout:this.updatePipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:this.fluxTex.createView()},{binding:4,resource:l},{binding:5,resource:u},{binding:6,resource:{buffer:this.theta0Buf}},{binding:7,resource:{buffer:this.twinCtrBuf}}]}),this.stampBG[r]=t.createBindGroup({layout:this.stampPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.seedBuf}},{binding:4,resource:l},{binding:5,resource:u}]}),this.statsBG[r]=t.createBindGroup({layout:this.statsPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.statsBuf}}]})}this.reset()}reset(t=this.params.tFar){const e=this.n;this.params.tFar=t;const a=new Float32Array(e*e*4),i=this.params.alloyOn?this.params.c0:0;for(let r=0;r<e*e;r++)a[r*4+1]=t,a[r*4+2]=i;for(const r of this.stateTex)this.device.queue.writeTexture({texture:r},a,{bytesPerRow:e*16},[e,e]);const s=new Uint32Array(e*e);for(const r of this.grainTex)this.device.queue.writeTexture({texture:r},s,{bytesPerRow:e*4},[e,e]);this.dir=0,this.frame=0,this.simTime=0,this.nextId=1,this.frontX=1,this.pendingSeeds=[],this.theta0CPU.fill(0),this.device.queue.writeBuffer(this.theta0Buf,0,this.theta0CPU),this.device.queue.writeBuffer(this.twinCtrBuf,0,new Uint32Array([d-1]))}addSeed(t,e,a=4,i,s=2){let r=this.nextId++;r>=d&&(this.nextId=2,r=1);const o=i??Math.random()*(2*Math.PI/this.params.aniMode);return this.theta0CPU[r]=o,this.device.queue.writeBuffer(this.theta0Buf,r*4,this.theta0CPU,r,1),this.pendingSeeds.push({x:t,y:e,r:a,id:r,tact:s}),r}quench(t=.25){this.pendingQuench+=t}addTwinSeed(t,e,a=4){const i=Math.random()*(2*Math.PI/this.params.aniMode),s=Math.random()*Math.PI*2,r=a*.45;this.addSeed(t-Math.cos(s)*r,e-Math.sin(s)*r,a,i),this.addSeed(t+Math.cos(s)*r,e+Math.sin(s)*r,a,i+Math.PI/this.params.aniMode)}chillWall(t="bottom",e=42){const a=this.n;for(let i=0;i<e;i++){const s=((i+.5)/e+(Math.random()-.5)*.6/e)*a;t==="bottom"?this.addSeed(s,a-3,3.5):this.addSeed(3,s,3.5)}}writeParams(t){const e=this.params,a=new Uint32Array(this.paramData),i=new Float32Array(this.paramData);a[0]=this.n,a[1]=this.frame,i[2]=e.dx,i[3]=e.dt,i[4]=e.epsBar,i[5]=e.delta,i[6]=e.aniMode,i[7]=e.tau,i[8]=e.alpha,i[9]=e.gamma,i[10]=e.latent,i[11]=e.noiseAmp,i[12]=e.tFar,i[13]=e.coolRate,i[14]=e.heatIn,a[15]=t,i[16]=this.simTime,a[17]=e.scen,i[18]=e.gradG,i[19]=this.frontX,i[20]=e.weldX,i[21]=e.weldY,i[22]=e.weldPow,i[23]=e.weldSig,a[24]=e.alloyOn,i[25]=e.c0,i[26]=e.mLiq,i[27]=e.kPart,i[28]=e.dSol,i[29]=this.pendingQuench,i[30]=e.twinProb,a[31]=this.nextId,this.device.queue.writeBuffer(this.paramBuf,0,this.paramData)}dispatch(t){const e=Math.ceil(this.n/8);t.dispatchWorkgroups(e,e)}step(t){if(this.busy)return 0;const e=this.device,a=Math.max(1,Math.floor(16e7/(this.n*this.n))),i=Math.min(t,a);let s=0;if(this.pendingSeeds.length>0){const u=this.pendingSeeds.splice(0,w),f=new Float32Array(u.length*x);u.forEach((g,m)=>{const h=m*x;f[h]=g.x,f[h+1]=g.y,f[h+2]=g.r,f[h+3]=g.id,f[h+4]=g.tact}),e.queue.writeBuffer(this.seedBuf,0,f),s=u.length}const r=s>0||this.pendingQuench!==0;if(i===0&&!r)return 0;this.frame++,this.params.scen===1&&(this.frontX=1+this.params.pullV*this.simTime),this.writeParams(s),this.pendingQuench=0;const o=e.createCommandEncoder(),c=o.beginComputePass();let l=this.dir;r&&(c.setPipeline(this.stampPipe),c.setBindGroup(0,this.stampBG[l]),this.dispatch(c),l=1-l);for(let u=0;u<i;u++)c.setPipeline(this.fluxPipe),c.setBindGroup(0,this.fluxBG[l]),this.dispatch(c),c.setPipeline(this.updatePipe),c.setBindGroup(0,this.updateBG[l]),this.dispatch(c),l=1-l;return c.end(),e.queue.submit([o.finish()]),this.dir=l,this.simTime+=i*this.params.dt,this.inFlight++,e.queue.onSubmittedWorkDone().then(()=>{this.inFlight=Math.max(0,this.inFlight-1)}),i}async readStats(){if(this.statsInFlight)return null;this.statsInFlight=!0;const t=this.device;this.writeParams(0);const e=t.createCommandEncoder();e.clearBuffer(this.statsBuf);const a=e.beginComputePass();a.setPipeline(this.statsPipe),a.setBindGroup(0,this.statsBG[this.dir]),this.dispatch(a),a.end(),e.copyBufferToBuffer(this.statsBuf,0,this.statsStaging,0,this.statsBuf.size),t.queue.submit([e.finish()]);try{await this.statsStaging.mapAsync(GPUMapMode.READ)}catch{return this.statsInFlight=!1,null}const i=new Uint32Array(this.statsStaging.getMappedRange().slice(0));this.statsStaging.unmap(),this.statsInFlight=!1;const s=this.n*this.n,r=i[0],o=i[1],c=o>0?i[2]/1e3/o:0,l=Math.max(20,this.n*this.n*1e-5),u=E*1e3/this.n,f=[];let g=0;for(let v=1;v<d;v++){const b=i[4+v];b>l&&(g+=b,f.push(2*Math.sqrt(b/Math.PI)*u))}const m=f.length,h=m>0?g/m:0;let y=null;if(m>=3&&h>0){const v=h*(u/1e3)**2;y=3.322*Math.log10(1/v)-2.954}return{fracSolid:r/s,grainCount:m,meanAreaPx:h,astm:y,interfaceT:c,diamsUm:f}}}class O{constructor(t,e,a){n(this,"device");n(this,"ctx");n(this,"canvas");n(this,"pipe");n(this,"rbuf");n(this,"bg",[]);n(this,"rdata",new ArrayBuffer(64));n(this,"zoom",1);n(this,"cx",.5);n(this,"cy",.5);n(this,"pixelSize",0);n(this,"paletteOn",!1);this.device=t,this.canvas=e,this.ctx=e.getContext("webgpu");const i=navigator.gpu.getPreferredCanvasFormat();this.ctx.configure({device:t,format:i,alphaMode:"opaque"}),this.pipe=t.createRenderPipeline({layout:"auto",vertex:{module:t.createShaderModule({code:S}),entryPoint:"vmain"},fragment:{module:t.createShaderModule({code:S}),entryPoint:"fmain",targets:[{format:i}]},primitive:{topology:"triangle-list"}}),this.rbuf=t.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.rebind(a)}rebind(t){for(const e of[0,1])this.bg[e]=this.device.createBindGroup({layout:this.pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.rbuf}},{binding:1,resource:t.stateTexture(e).createView()},{binding:2,resource:t.grainTexture(e).createView()},{binding:3,resource:{buffer:t.theta0Buffer}}]})}resize(){const t=Math.min(window.devicePixelRatio||1,2),e=Math.floor(this.canvas.clientWidth*t),a=Math.floor(this.canvas.clientHeight*t);(this.canvas.width!==e||this.canvas.height!==a)&&(this.canvas.width=e,this.canvas.height=a)}clampView(){this.zoom=Math.min(24,Math.max(1,this.zoom));const t=.5/this.zoom;this.cx=Math.min(1-t,Math.max(t,this.cx)),this.cy=Math.min(1-t,Math.max(t,this.cy))}resetView(){this.zoom=1,this.cx=.5,this.cy=.5}clientToCover(t,e){const a=this.canvas.getBoundingClientRect(),i=this.canvas.width/a.width,s=(t-a.left)*i,r=(e-a.top)*i,o=Math.max(this.canvas.width,this.canvas.height),c=(this.canvas.width-o)*.5,l=(this.canvas.height-o)*.5;return{x:(s-c)/o,y:(r-l)/o}}clientToGrid(t,e,a){const i=this.clientToCover(t,e),s=((i.x-.5)/this.zoom+this.cx)*a,r=((i.y-.5)/this.zoom+this.cy)*a;return s<0||r<0||s>=a||r>=a?null:{x:s,y:r}}zoomAt(t,e,a){const i=this.clientToCover(t,e),s=(i.x-.5)/this.zoom+this.cx,r=(i.y-.5)/this.zoom+this.cy;this.zoom*=a,this.zoom=Math.min(24,Math.max(1,this.zoom)),this.cx=s-(i.x-.5)/this.zoom,this.cy=r-(i.y-.5)/this.zoom,this.clampView()}panBy(t,e){const a=this.canvas.getBoundingClientRect(),i=this.canvas.width/a.width,s=Math.max(this.canvas.width,this.canvas.height);this.cx-=t*i/s/this.zoom,this.cy-=e*i/s/this.zoom,this.clampView()}cssPxPerCell(t){const e=this.canvas.getBoundingClientRect();return Math.max(e.width,e.height)/t*this.zoom}render(t,e,a){this.resize();const i=new Uint32Array(this.rdata),s=new Float32Array(this.rdata);i[0]=e,i[1]=t.n,s[2]=this.canvas.width,s[3]=this.canvas.height,s[4]=a,s[5]=t.params.aniMode,s[6]=t.params.tFar,s[7]=this.zoom,s[8]=this.cx,s[9]=this.cy,s[10]=this.pixelSize,i[11]=this.paletteOn?1:0,i[12]=t.params.alloyOn,s[13]=t.params.c0,s[14]=t.params.meltGlow,this.device.queue.writeBuffer(this.rbuf,0,this.rdata);const r=this.device.createCommandEncoder(),o=r.beginRenderPass({colorAttachments:[{view:this.ctx.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:.04,g:.04,b:.05,a:1},storeOp:"store"}]});o.setPipeline(this.pipe),o.setBindGroup(0,this.bg[t.dir]),o.draw(3),o.end(),this.device.queue.submit([r.finish()])}}export{E as D,_ as L,O as R,C as S};

var M=Object.defineProperty;var R=(m,t,e)=>t in m?M(m,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):m[t]=e;var n=(m,t,e)=>R(m,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function e(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=e(i);fetch(i.href,a)}})();const f=4096,T=64,b=6,y=`
struct Params {
  n: u32,
  frame: u32,
  dx: f32,
...
`,O=["MELT","ORIENT","ETCH","FIELD","RINGS","THERM","SEM","NEON","XRAY","CURV"],B=`
struct RParams {
  view: u32,
  n: u32,
  canvasW: f32,
...
`,E={dx:.03,dt:12e-5,epsBar:.01,delta:.04,aniMode:4,tau:3e-4,alpha:.9,gamma:10,latent:1.6,noiseAmp:.01,tFar:0,coolRate:0,heatIn:0,scen:0,gradG:.08,pullV:1.5,weldX:0,weldY:0,weldPow:700,weldSig:4,alloyOn:0,c0:.3,mLiq:.45,kPart:.2,dSol:.8,twinProb:0,meltGlow:1},C=1;class N{constructor(t,e){n(this,"device");n(this,"n");n(this,"params");n(this,"frame",0);n(this,"simTime",0);n(this,"dir",0);n(this,"nextId",1);n(this,"frontX",0);n(this,"probe",null);n(this,"stateTex",[]);n(this,"grainTex",[]);n(this,"fluxTex");n(this,"paramBuf");n(this,"theta0Buf");n(this,"twinCtrBuf");n(this,"seedBuf");n(this,"statsBuf");n(this,"statsStaging");n(this,"theta0CPU",new Float32Array(f));n(this,"fluxPipe");n(this,"updatePipe");n(this,"stampPipe");n(this,"statsPipe");n(this,"fluxBG",[]);n(this,"updateBG",[]);n(this,"stampBG",[]);n(this,"statsBG",[]);n(this,"pendingSeeds",[]);n(this,"pendingQuench",0);n(this,"statsInFlight",!1);n(this,"paramData",new ArrayBuffer(144));n(this,"inFlight",0);this.device=t,this.n=e,this.params={...E},this.build()}get busy(){return this.inFlight>=2}get theta0Buffer(){return this.theta0Buf}stateTexture(t){return this.stateTex[t]}grainTexture(t){return this.grainTex[t]}build(){const t=this.device,e=this.n,s=r=>({size:[e,e],format:r,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.COPY_SRC});this.stateTex=[t.createTexture(s("rgba32float")),t.createTexture(s("rgba32float"))],this.grainTex=[t.createTexture(s("r32uint")),t.createTexture(s("r32uint"))],this.fluxTex=t.createTexture(s("rgba32float")),this.paramBuf=t.createBuffer({size:144,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.theta0Buf=t.createBuffer({size:f*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.twinCtrBuf=t.createBuffer({size:4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.seedBuf=t.createBuffer({size:T*b*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});const i=(8+f)*4;this.statsBuf=t.createBuffer({size:i,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.statsStaging=t.createBuffer({size:i,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});const a=r=>t.createComputePipeline({layout:"auto",compute:{module:t.createShaderModule({code:r}),entryPoint:"main"}});this.fluxPipe=a(G),this.updatePipe=a(L),this.stampPipe=a(U),this.statsPipe=a(z);for(const r of[0,1]){const o=this.stateTex[r].createView(),c=this.grainTex[r].createView(),l=this.stateTex[1-r].createView(),u=this.grainTex[1-r].createView();this.fluxBG[r]=t.createBindGroup({layout:this.fluxPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.theta0Buf}},{binding:4,resource:this.fluxTex.createView()}]}),this.updateBG[r]=t.createBindGroup({layout:this.updatePipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:this.fluxTex.createView()},{binding:4,resource:l},{binding:5,resource:u},{binding:6,resource:{buffer:this.theta0Buf}},{binding:7,resource:{buffer:this.twinCtrBuf}}]}),this.stampBG[r]=t.createBindGroup({layout:this.stampPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.seedBuf}},{binding:4,resource:l},{binding:5,resource:u}]}),this.statsBG[r]=t.createBindGroup({layout:this.statsPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:o},{binding:2,resource:c},{binding:3,resource:{buffer:this.statsBuf}}]})}this.reset()}reset(t=this.params.tFar){const e=this.n;this.params.tFar=t;const s=new Float32Array(e*e*4),i=this.params.alloyOn?this.params.c0:0;for(let r=0;r<e*e;r++)s[r*4+1]=t,s[r*4+2]=i;for(const r of this.stateTex)this.device.queue.writeTexture({texture:r},s,{bytesPerRow:e*16},[e,e]);const a=new Uint32Array(e*e);for(const r of this.grainTex)this.device.queue.writeTexture({texture:r},a,{bytesPerRow:e*4},[e,e]);this.dir=0,this.frame=0,this.simTime=0,this.nextId=1,this.frontX=1,this.pendingSeeds=[],this.theta0CPU.fill(0),this.device.queue.writeBuffer(this.theta0Buf,0,this.theta0CPU),this.device.queue.writeBuffer(this.twinCtrBuf,0,new Uint32Array([f-1]))}addSeed(t,e,s=4,i,a=2){let r=this.nextId++;r>=f&&(this.nextId=2,r=1);const o=i??Math.random()*(2*Math.PI/this.params.aniMode);return this.theta0CPU[r]=o,this.device.queue.writeBuffer(this.theta0Buf,r*4,this.theta0CPU,r,1),this.pendingSeeds.push({x:t,y:e,r:s,id:r,tact:a}),r}quench(t=.25){this.pendingQuench+=t}addTwinSeed(t,e,s=4){const i=Math.random()*(2*Math.PI/this.params.aniMode),a=Math.random()*Math.PI*2,r=s*.45;this.addSeed(t-Math.cos(a)*r,e-Math.sin(a)*r,s,i),this.addSeed(t+Math.cos(a)*r,e+Math.sin(a)*r,s,i+Math.PI/this.params.aniMode)}chillWall(t="bottom",e=42){const s=this.n;for(let i=0;i<e;i++){const a=((i+.5)/e+(Math.random()-.5)*.6/e)*s;t==="bottom"?this.addSeed(a,s-3,3.5):this.addSeed(3,a,3.5)}}writeParams(t){const e=this.params,s=new Uint32Array(this.paramData),i=new Float32Array(this.paramData);s[0]=this.n,s[1]=this.frame,i[2]=e.dx,i[3]=e.dt,i[4]=e.epsBar,i[5]=e.delta,i[6]=e.aniMode,i[7]=e.tau,i[8]=e.alpha,i[9]=e.gamma,i[10]=e.latent,i[11]=e.noiseAmp,i[12]=e.tFar,i[13]=e.coolRate,i[14]=e.heatIn,s[15]=t,i[16]=this.simTime,s[17]=e.scen,i[18]=e.gradG,i[19]=this.frontX,i[20]=e.weldX,i[21]=e.weldY,i[22]=e.weldPow,i[23]=e.weldSig,s[24]=e.alloyOn,i[25]=e.c0,i[26]=e.mLiq,i[27]=e.kPart,i[28]=e.dSol,i[29]=this.pendingQuench,i[30]=e.twinProb,s[31]=this.nextId,s[32]=this.probe?Math.round(this.probe.x):4294967295,s[33]=this.probe?Math.round(this.probe.y):4294967295,this.device.queue.writeBuffer(this.paramBuf,0,this.paramData)}dispatch(t){const e=Math.ceil(this.n/8);t.dispatchWorkgroups(e,e)}step(t){if(this.busy)return 0;const e=this.device,s=Math.max(1,Math.floor(16e7/(this.n*this.n))),i=Math.min(t,s);let a=0;if(this.pendingSeeds.length>0){const u=this.pendingSeeds.splice(0,T),d=new Float32Array(u.length*b);u.forEach((h,v)=>{const p=v*b;d[p]=h.x,d[p+1]=h.y,d[p+2]=h.r,d[p+3]=h.id,d[p+4]=h.tact}),e.queue.writeBuffer(this.seedBuf,0,d),a=u.length}const r=a>0||this.pendingQuench!==0;if(i===0&&!r)return 0;this.frame++,this.params.scen===1&&(this.frontX=1+this.params.pullV*this.simTime),this.writeParams(a),this.pendingQuench=0;const o=e.createCommandEncoder(),c=o.beginComputePass();let l=this.dir;r&&(c.setPipeline(this.stampPipe),c.setBindGroup(0,this.stampBG[l]),this.dispatch(c),l=1-l);for(let u=0;u<i;u++)c.setPipeline(this.fluxPipe),c.setBindGroup(0,this.fluxBG[l]),this.dispatch(c),c.setPipeline(this.updatePipe),c.setBindGroup(0,this.updateBG[l]),this.dispatch(c),l=1-l;return c.end(),e.queue.submit([o.finish()]),this.dir=l,this.simTime+=i*this.params.dt,this.inFlight++,e.queue.onSubmittedWorkDone().then(()=>{this.inFlight=Math.max(0,this.inFlight-1)}),i}async readStats(){if(this.statsInFlight)return null;this.statsInFlight=!0;const t=this.device;this.writeParams(0);const e=t.createCommandEncoder();e.clearBuffer(this.statsBuf);const s=e.beginComputePass();s.setPipeline(this.statsPipe),s.setBindGroup(0,this.statsBG[this.dir]),this.dispatch(s),s.end(),e.copyBufferToBuffer(this.statsBuf,0,this.statsStaging,0,this.statsBuf.size),t.queue.submit([e.finish()]);try{await this.statsStaging.mapAsync(GPUMapMode.READ)}catch{return this.statsInFlight=!1,null}const i=new Uint32Array(this.statsStaging.getMappedRange().slice(0));this.statsStaging.unmap(),this.statsInFlight=!1;const a=this.n*this.n,r=i[0],o=i[1],c=o>0?i[2]/1e3/o:0,l=this.probe?i[4]/1e3-1:null,u=this.probe?i[5]/1e3:null,d=Math.max(20,this.n*this.n*1e-5),h=C*1e3/this.n,v=[];let p=0;for(let x=1;x<f;x++){const S=i[8+x];S>d&&(p+=S,v.push(2*Math.sqrt(S/Math.PI)*h))}const g=v.length,P=g>0?p/g:0;let w=null;if(g>=3&&P>0){const x=P*(h/1e3)**2;w=3.322*Math.log10(1/x)-2.954}return{fracSolid:r/a,grainCount:g,meanAreaPx:P,astm:w,interfaceT:c,diamsUm:v,probeT:l,probePhi:u}}async readLine(t,e,s,i,a=400){const r=this.n,o=g=>Math.min(r-1,Math.max(0,g));t=o(t),e=o(e),s=o(s),i=o(i);const c=Math.floor(Math.min(e,i)),l=Math.ceil(Math.max(e,i))-c+1,u=r*16,d=this.device.createBuffer({size:u*l,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),h=this.device.createCommandEncoder();h.copyTextureToBuffer({texture:this.stateTex[this.dir],origin:{x:0,y:c}},{buffer:d,bytesPerRow:u},{width:r,height:l}),this.device.queue.submit([h.finish()]);try{await d.mapAsync(GPUMapMode.READ)}catch{return d.destroy(),null}const v=new Float32Array(d.getMappedRange().slice(0));d.unmap(),d.destroy();const p=new Float32Array(a);for(let g=0;g<a;g++){const P=g/(a-1),w=Math.round(t+(s-t)*P),x=Math.round(e+(i-e)*P);p[g]=v[((x-c)*r+w)*4]}return p}}class A{constructor(t,e,s){n(this,"device");n(this,"ctx");n(this,"canvas");n(this,"pipe");n(this,"rbuf");n(this,"bg",[]);n(this,"rdata",new ArrayBuffer(64));n(this,"zoom",1);n(this,"cx",.5);n(this,"cy",.5);n(this,"pixelSize",0);n(this,"paletteOn",!1);this.device=t,this.canvas=e,this.ctx=e.getContext("webgpu");const i=navigator.gpu.getPreferredCanvasFormat();this.ctx.configure({device:t,format:i,alphaMode:"opaque"}),this.pipe=t.createRenderPipeline({layout:"auto",vertex:{module:t.createShaderModule({code:B}),entryPoint:"vmain"},fragment:{module:t.createShaderModule({code:B}),entryPoint:"fmain",targets:[{format:i}]},primitive:{topology:"triangle-list"}}),this.rbuf=t.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.rebind(s)}rebind(t){for(const e of[0,1])this.bg[e]=this.device.createBindGroup({layout:this.pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.rbuf}},{binding:1,resource:t.stateTexture(e).createView()},{binding:2,resource:t.grainTexture(e).createView()},{binding:3,resource:{buffer:t.theta0Buffer}}]})}resize(){const t=Math.min(window.devicePixelRatio||1,2),e=Math.floor(this.canvas.clientWidth*t),s=Math.floor(this.canvas.clientHeight*t);(this.canvas.width!==e||this.canvas.height!==s)&&(this.canvas.width=e,this.canvas.height=s)}clampView(){this.zoom=Math.min(24,Math.max(1,this.zoom));const t=.5/this.zoom;this.cx=Math.min(1-t,Math.max(t,this.cx)),this.cy=Math.min(1-t,Math.max(t,this.cy))}resetView(){this.zoom=1,this.cx=.5,this.cy=.5}clientToCover(t,e){const s=this.canvas.getBoundingClientRect(),i=this.canvas.width/s.width,a=(t-s.left)*i,r=(e-s.top)*i,o=Math.max(this.canvas.width,this.canvas.height),c=(this.canvas.width-o)*.5,l=(this.canvas.height-o)*.5;return{x:(a-c)/o,y:(r-l)/o}}clientToGrid(t,e,s){const i=this.clientToCover(t,e),a=((i.x-.5)/this.zoom+this.cx)*s,r=((i.y-.5)/this.zoom+this.cy)*s;return a<0||r<0||a>=s||r>=s?null:{x:a,y:r}}zoomAt(t,e,s){const i=this.clientToCover(t,e),a=(i.x-.5)/this.zoom+this.cx,r=(i.y-.5)/this.zoom+this.cy;this.zoom*=s,this.zoom=Math.min(24,Math.max(1,this.zoom)),this.cx=a-(i.x-.5)/this.zoom,this.cy=r-(i.y-.5)/this.zoom,this.clampView()}panBy(t,e){const s=this.canvas.getBoundingClientRect(),i=this.canvas.width/s.width,a=Math.max(this.canvas.width,this.canvas.height);this.cx-=t*i/a/this.zoom,this.cy-=e*i/a/this.zoom,this.clampView()}gridToClient(t,e,s){const i=this.canvas.getBoundingClientRect(),a=this.canvas.width/Math.max(i.width,1),r=Math.max(this.canvas.width,this.canvas.height),o=(this.canvas.width-r)*.5,c=(this.canvas.height-r)*.5,l=(t/s-this.cx)*this.zoom+.5,u=(e/s-this.cy)*this.zoom+.5;return{x:(l*r+o)/a+i.left,y:(u*r+c)/a+i.top}}cssPxPerCell(t){const e=this.canvas.getBoundingClientRect();return Math.max(e.width,e.height)/t*this.zoom}render(t,e,s){this.resize();const i=new Uint32Array(this.rdata),a=new Float32Array(this.rdata);i[0]=e,i[1]=t.n,a[2]=this.canvas.width,a[3]=this.canvas.height,a[4]=s,a[5]=t.params.aniMode,a[6]=t.params.tFar,a[7]=this.zoom,a[8]=this.cx,a[9]=this.cy,a[10]=this.pixelSize,i[11]=this.paletteOn?1:0,i[12]=t.params.alloyOn,a[13]=t.params.c0,a[14]=t.params.meltGlow,this.device.queue.writeBuffer(this.rbuf,0,this.rdata);const r=this.device.createCommandEncoder(),o=r.beginRenderPass({colorAttachments:[{view:this.ctx.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:.04,g:.04,b:.05,a:1},storeOp:"store"}]});o.setPipeline(this.pipe),o.setBindGroup(0,this.bg[t.dir]),o.draw(3),o.end(),this.device.queue.submit([r.finish()])}}export{C as D,O as L,A as R,N as S};

var T=Object.defineProperty;var B=(p,t,e)=>t in p?T(p,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):p[t]=e;var o=(p,t,e)=>B(p,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))s(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const r of a.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&s(r)}).observe(document,{childList:!0,subtree:!0});function e(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function s(i){if(i.ep)return;i.ep=!0;const a=e(i);fetch(i.href,a)}})();const h=4096,y=64,x=6,P=`
struct Params {
  n: u32,
  frame: u32,
  dx: f32,
...
@group(0) @binding(1) var state: texture_2d<f32>;
@group(0) @binding(2) var grain: texture_2d<u32>;
@group(0) @binding(3) var<storage, read> seeds: array<f32, ${y*x}>;
@group(0) @binding(4) var stateOut: texture_storage_2d<rgba32float, write>;
@group(0) @binding(5) var grainOut: texture_storage_2d<r32uint, write>;
...
`,E={dx:.03,dt:12e-5,epsBar:.01,delta:.04,aniMode:4,tau:3e-4,alpha:.9,gamma:10,latent:1.6,noiseAmp:.01,tFar:0,coolRate:0,heatIn:0,scen:0,gradG:.08,pullV:1.5,weldX:0,weldY:0,weldPow:700,weldSig:4,alloyOn:0,c0:.3,mLiq:.45,kPart:.2,dSol:.8},z=1;class _{constructor(t,e){o(this,"device");o(this,"n");o(this,"params");o(this,"frame",0);o(this,"simTime",0);o(this,"dir",0);o(this,"nextId",1);o(this,"frontX",0);o(this,"stateTex",[]);o(this,"grainTex",[]);o(this,"fluxTex");o(this,"paramBuf");o(this,"theta0Buf");o(this,"seedBuf");o(this,"statsBuf");o(this,"statsStaging");o(this,"theta0CPU",new Float32Array(h));o(this,"fluxPipe");o(this,"updatePipe");o(this,"stampPipe");o(this,"statsPipe");o(this,"fluxBG",[]);o(this,"updateBG",[]);o(this,"stampBG",[]);o(this,"statsBG",[]);o(this,"pendingSeeds",[]);o(this,"pendingQuench",0);o(this,"statsInFlight",!1);o(this,"paramData",new ArrayBuffer(128));o(this,"inFlight",0);this.device=t,this.n=e,this.params={...E},this.build()}get busy(){return this.inFlight>=2}get theta0Buffer(){return this.theta0Buf}stateTexture(t){return this.stateTex[t]}grainTexture(t){return this.grainTex[t]}build(){const t=this.device,e=this.n,s=r=>({size:[e,e],format:r,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST});this.stateTex=[t.createTexture(s("rgba32float")),t.createTexture(s("rgba32float"))],this.grainTex=[t.createTexture(s("r32uint")),t.createTexture(s("r32uint"))],this.fluxTex=t.createTexture(s("rgba32float")),this.paramBuf=t.createBuffer({size:128,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.theta0Buf=t.createBuffer({size:h*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.seedBuf=t.createBuffer({size:y*x*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});const i=(4+h)*4;this.statsBuf=t.createBuffer({size:i,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.statsStaging=t.createBuffer({size:i,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});const a=r=>t.createComputePipeline({layout:"auto",compute:{module:t.createShaderModule({code:r}),entryPoint:"main"}});this.fluxPipe=a(R),this.updatePipe=a(L),this.stampPipe=a(G),this.statsPipe=a(M);for(const r of[0,1]){const n=this.stateTex[r].createView(),c=this.grainTex[r].createView(),u=this.stateTex[1-r].createView(),l=this.grainTex[1-r].createView();this.fluxBG[r]=t.createBindGroup({layout:this.fluxPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.theta0Buf}},{binding:4,resource:this.fluxTex.createView()}]}),this.updateBG[r]=t.createBindGroup({layout:this.updatePipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:this.fluxTex.createView()},{binding:4,resource:u},{binding:5,resource:l}]}),this.stampBG[r]=t.createBindGroup({layout:this.stampPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.seedBuf}},{binding:4,resource:u},{binding:5,resource:l}]}),this.statsBG[r]=t.createBindGroup({layout:this.statsPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.statsBuf}}]})}this.reset()}reset(t=this.params.tFar){const e=this.n;this.params.tFar=t;const s=new Float32Array(e*e*4),i=this.params.alloyOn?this.params.c0:0;for(let r=0;r<e*e;r++)s[r*4+1]=t,s[r*4+2]=i;for(const r of this.stateTex)this.device.queue.writeTexture({texture:r},s,{bytesPerRow:e*16},[e,e]);const a=new Uint32Array(e*e);for(const r of this.grainTex)this.device.queue.writeTexture({texture:r},a,{bytesPerRow:e*4},[e,e]);this.dir=0,this.frame=0,this.simTime=0,this.nextId=1,this.frontX=1,this.pendingSeeds=[],this.theta0CPU.fill(0),this.device.queue.writeBuffer(this.theta0Buf,0,this.theta0CPU)}addSeed(t,e,s=4,i,a=2){let r=this.nextId++;r>=h&&(this.nextId=2,r=1);const n=i??Math.random()*(2*Math.PI/this.params.aniMode);return this.theta0CPU[r]=n,this.device.queue.writeBuffer(this.theta0Buf,r*4,this.theta0CPU,r,1),this.pendingSeeds.push({x:t,y:e,r:s,id:r,tact:a}),r}quench(t=.25){this.pendingQuench+=t}chillWall(t="bottom",e=42){const s=this.n;for(let i=0;i<e;i++){const a=((i+.5)/e+(Math.random()-.5)*.6/e)*s;t==="bottom"?this.addSeed(a,s-3,3.5):this.addSeed(3,a,3.5)}}writeParams(t){const e=this.params,s=new Uint32Array(this.paramData),i=new Float32Array(this.paramData);s[0]=this.n,s[1]=this.frame,i[2]=e.dx,i[3]=e.dt,i[4]=e.epsBar,i[5]=e.delta,i[6]=e.aniMode,i[7]=e.tau,i[8]=e.alpha,i[9]=e.gamma,i[10]=e.latent,i[11]=e.noiseAmp,i[12]=e.tFar,i[13]=e.coolRate,i[14]=e.heatIn,s[15]=t,i[16]=this.simTime,s[17]=e.scen,i[18]=e.gradG,i[19]=this.frontX,i[20]=e.weldX,i[21]=e.weldY,i[22]=e.weldPow,i[23]=e.weldSig,s[24]=e.alloyOn,i[25]=e.c0,i[26]=e.mLiq,i[27]=e.kPart,i[28]=e.dSol,i[29]=this.pendingQuench,this.device.queue.writeBuffer(this.paramBuf,0,this.paramData)}dispatch(t){const e=Math.ceil(this.n/8);t.dispatchWorkgroups(e,e)}step(t){if(this.busy)return 0;const e=this.device,s=Math.max(1,Math.floor(16e7/(this.n*this.n))),i=Math.min(t,s);let a=0;if(this.pendingSeeds.length>0){const l=this.pendingSeeds.splice(0,y),d=new Float32Array(l.length*x);l.forEach((g,m)=>{const f=m*x;d[f]=g.x,d[f+1]=g.y,d[f+2]=g.r,d[f+3]=g.id,d[f+4]=g.tact}),e.queue.writeBuffer(this.seedBuf,0,d),a=l.length}const r=a>0||this.pendingQuench!==0;if(i===0&&!r)return 0;this.frame++,this.params.scen===1&&(this.frontX=1+this.params.pullV*this.simTime),this.writeParams(a),this.pendingQuench=0;const n=e.createCommandEncoder(),c=n.beginComputePass();let u=this.dir;r&&(c.setPipeline(this.stampPipe),c.setBindGroup(0,this.stampBG[u]),this.dispatch(c),u=1-u);for(let l=0;l<i;l++)c.setPipeline(this.fluxPipe),c.setBindGroup(0,this.fluxBG[u]),this.dispatch(c),c.setPipeline(this.updatePipe),c.setBindGroup(0,this.updateBG[u]),this.dispatch(c),u=1-u;return c.end(),e.queue.submit([n.finish()]),this.dir=u,this.simTime+=i*this.params.dt,this.inFlight++,e.queue.onSubmittedWorkDone().then(()=>{this.inFlight=Math.max(0,this.inFlight-1)}),i}async readStats(){if(this.statsInFlight)return null;this.statsInFlight=!0;const t=this.device;this.writeParams(0);const e=t.createCommandEncoder();e.clearBuffer(this.statsBuf);const s=e.beginComputePass();s.setPipeline(this.statsPipe),s.setBindGroup(0,this.statsBG[this.dir]),this.dispatch(s),s.end(),e.copyBufferToBuffer(this.statsBuf,0,this.statsStaging,0,this.statsBuf.size),t.queue.submit([e.finish()]);try{await this.statsStaging.mapAsync(GPUMapMode.READ)}catch{return this.statsInFlight=!1,null}const i=new Uint32Array(this.statsStaging.getMappedRange().slice(0));this.statsStaging.unmap(),this.statsInFlight=!1;const a=this.n*this.n,r=i[0],n=i[1],c=n>0?i[2]/1e3/n:0,u=Math.max(20,this.n*this.n*1e-5),l=z*1e3/this.n,d=[];let g=0;for(let v=1;v<h;v++){const b=i[4+v];b>u&&(g+=b,d.push(2*Math.sqrt(b/Math.PI)*l))}const m=d.length,f=m>0?g/m:0;let w=null;if(m>=3&&f>0){const v=f*(l/1e3)**2;w=3.322*Math.log10(1/v)-2.954}return{fracSolid:r/a,grainCount:m,meanAreaPx:f,astm:w,interfaceT:c,diamsUm:d}}}class U{constructor(t,e,s){o(this,"device");o(this,"ctx");o(this,"canvas");o(this,"pipe");o(this,"rbuf");o(this,"bg",[]);o(this,"rdata",new ArrayBuffer(64));o(this,"zoom",1);o(this,"cx",.5);o(this,"cy",.5);o(this,"pixelSize",0);o(this,"paletteOn",!1);this.device=t,this.canvas=e,this.ctx=e.getContext("webgpu");const i=navigator.gpu.getPreferredCanvasFormat();this.ctx.configure({device:t,format:i,alphaMode:"opaque"}),this.pipe=t.createRenderPipeline({layout:"auto",vertex:{module:t.createShaderModule({code:S}),entryPoint:"vmain"},fragment:{module:t.createShaderModule({code:S}),entryPoint:"fmain",targets:[{format:i}]},primitive:{topology:"triangle-list"}}),this.rbuf=t.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.rebind(s)}rebind(t){for(const e of[0,1])this.bg[e]=this.device.createBindGroup({layout:this.pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.rbuf}},{binding:1,resource:t.stateTexture(e).createView()},{binding:2,resource:t.grainTexture(e).createView()},{binding:3,resource:{buffer:t.theta0Buffer}}]})}resize(){const t=Math.min(window.devicePixelRatio||1,2),e=Math.floor(this.canvas.clientWidth*t),s=Math.floor(this.canvas.clientHeight*t);(this.canvas.width!==e||this.canvas.height!==s)&&(this.canvas.width=e,this.canvas.height=s)}clampView(){this.zoom=Math.min(24,Math.max(1,this.zoom));const t=.5/this.zoom;this.cx=Math.min(1-t,Math.max(t,this.cx)),this.cy=Math.min(1-t,Math.max(t,this.cy))}resetView(){this.zoom=1,this.cx=.5,this.cy=.5}clientToCover(t,e){const s=this.canvas.getBoundingClientRect(),i=this.canvas.width/s.width,a=(t-s.left)*i,r=(e-s.top)*i,n=Math.max(this.canvas.width,this.canvas.height),c=(this.canvas.width-n)*.5,u=(this.canvas.height-n)*.5;return{x:(a-c)/n,y:(r-u)/n}}clientToGrid(t,e,s){const i=this.clientToCover(t,e),a=((i.x-.5)/this.zoom+this.cx)*s,r=((i.y-.5)/this.zoom+this.cy)*s;return a<0||r<0||a>=s||r>=s?null:{x:a,y:r}}zoomAt(t,e,s){const i=this.clientToCover(t,e),a=(i.x-.5)/this.zoom+this.cx,r=(i.y-.5)/this.zoom+this.cy;this.zoom*=s,this.zoom=Math.min(24,Math.max(1,this.zoom)),this.cx=a-(i.x-.5)/this.zoom,this.cy=r-(i.y-.5)/this.zoom,this.clampView()}panBy(t,e){const s=this.canvas.getBoundingClientRect(),i=this.canvas.width/s.width,a=Math.max(this.canvas.width,this.canvas.height);this.cx-=t*i/a/this.zoom,this.cy-=e*i/a/this.zoom,this.clampView()}cssPxPerCell(t){const e=this.canvas.getBoundingClientRect();return Math.max(e.width,e.height)/t*this.zoom}render(t,e,s){this.resize();const i=new Uint32Array(this.rdata),a=new Float32Array(this.rdata);i[0]=e,i[1]=t.n,a[2]=this.canvas.width,a[3]=this.canvas.height,a[4]=s,a[5]=t.params.aniMode,a[6]=t.params.tFar,a[7]=this.zoom,a[8]=this.cx,a[9]=this.cy,a[10]=this.pixelSize,i[11]=this.paletteOn?1:0,i[12]=t.params.alloyOn,a[13]=t.params.c0,this.device.queue.writeBuffer(this.rbuf,0,this.rdata);const r=this.device.createCommandEncoder(),n=r.beginRenderPass({colorAttachments:[{view:this.ctx.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:.04,g:.04,b:.05,a:1},storeOp:"store"}]});n.setPipeline(this.pipe),n.setBindGroup(0,this.bg[t.dir]),n.draw(3),n.end(),this.device.queue.submit([r.finish()])}}export{z as D,O as L,U as R,_ as S};

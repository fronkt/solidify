...
fn main(@builtin(global_invocation_id) gid: vec3u) {
  ...
  if (phiNew < 1e-4) {
    ...
    if (id != 0u && best > 0.003 && P.twinProb > 0.0 &&
      ...
      if (tid > P.idFloor && tid < ${d}u) {
        theta0[tid] = theta0[min(id, ${d-1}u)] + PI / max(P.aniMode, 1.0);
        id = tid;
      }
    }
  }
...
@group(0) @binding(1) var state: texture_2d<f32>;
@group(0) @binding(2) var grain: texture_2d<u32>;
@group(0) @binding(3) var<storage, read> seeds: array<f32, ${R*P}>;
@group(0) @binding(4) var stateOut: texture_storage_2d<rgba32float, write>;
@group(0) @binding(5) var grainOut: texture_storage_2d<r32uint, write>;
...
`,k=["MELT","ORIENT","ETCH","FIELD","RINGS","THERM","SEM","NEON","XRAY","CURV"],G=`
struct RParams {
  view: u32,
  n: u32,
  canvasW: f32,
  ...
  if (id != 0u && phi > 0.5) {
    ...
    for (var k = 0; k < 4; k++) {
      ...
      if (nid != 0u && nid != id && textureLoad(state, nc, 0).r > 0.5) {
        gb = 1.0;
        let dth = theta0[min(nid, ${d-1}u)] - th0;
...
`,_={dx:.03,dt:12e-5,epsBar:.01,delta:.04,aniMode:4,tau:3e-4,alpha:.9,gamma:10,latent:1.6,noiseAmp:.01,tFar:0,coolRate:0,heatIn:0,scen:0,gradG:.08,pullV:1.5,weldX:0,weldY:0,weldPow:700,weldSig:4,alloyOn:0,c0:.3,mLiq:.45,kPart:.2,dSol:.8,twinProb:0,meltGlow:1,holdT:0,holdRate:0},I=1;class F{constructor(i,t){o(this,"device");o(this,"n");o(this,"params");o(this,"frame",0);o(this,"simTime",0);o(this,"dir",0);o(this,"nextId",1);o(this,"frontX",0);o(this,"probe",null);o(this,"stateTex",[]);o(this,"grainTex",[]);o(this,"fluxTex");o(this,"paramBuf");o(this,"theta0Buf");o(this,"twinCtrBuf");o(this,"seedBuf");o(this,"statsBuf");o(this,"statsStaging");o(this,"theta0CPU",new Float32Array(d));o(this,"fluxPipe");o(this,"updatePipe");o(this,"stampPipe");o(this,"statsPipe");o(this,"fluxBG",[]);o(this,"updateBG",[]);o(this,"stampBG",[]);o(this,"statsBG",[]);o(this,"pendingSeeds",[]);o(this,"pendingQuench",0);o(this,"statsInFlight",!1);o(this,"paramData",new ArrayBuffer(144));o(this,"inFlight",0);this.device=i,this.n=t,this.params={..._},this.build()}get busy(){return this.inFlight>=2}get theta0Buffer(){return this.theta0Buf}stateTexture(i){return this.stateTex[i]}grainTexture(i){return this.grainTex[i]}build(){const i=this.device,t=this.n,s=r=>({size:[t,t],format:r,usage:GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.STORAGE_BINDING|GPUTextureUsage.COPY_DST|GPUTextureUsage.COPY_SRC});this.stateTex=[i.createTexture(s("rgba32float")),i.createTexture(s("rgba32float"))],this.grainTex=[i.createTexture(s("r32uint")),i.createTexture(s("r32uint"))],this.fluxTex=i.createTexture(s("rgba32float")),this.paramBuf=i.createBuffer({size:144,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.theta0Buf=i.createBuffer({size:d*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC}),this.twinCtrBuf=i.createBuffer({size:4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.seedBuf=i.createBuffer({size:R*P*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});const e=(8+d)*4;this.statsBuf=i.createBuffer({size:e,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.statsStaging=i.createBuffer({size:e+d*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});const a=r=>i.createComputePipeline({layout:"auto",compute:{module:i.createShaderModule({code:r}),entryPoint:"main"}});this.fluxPipe=a(O),this.updatePipe=a(A),this.stampPipe=a(E),this.statsPipe=a(N);for(const r of[0,1]){const n=this.stateTex[r].createView(),c=this.grainTex[r].createView(),l=this.stateTex[1-r].createView(),u=this.grainTex[1-r].createView();this.fluxBG[r]=i.createBindGroup({layout:this.fluxPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.theta0Buf}},{binding:4,resource:this.fluxTex.createView()}]}),this.updateBG[r]=i.createBindGroup({layout:this.updatePipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:this.fluxTex.createView()},{binding:4,resource:l},{binding:5,resource:u},{binding:6,resource:{buffer:this.theta0Buf}},{binding:7,resource:{buffer:this.twinCtrBuf}}]}),this.stampBG[r]=i.createBindGroup({layout:this.stampPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.seedBuf}},{binding:4,resource:l},{binding:5,resource:u}]}),this.statsBG[r]=i.createBindGroup({layout:this.statsPipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramBuf}},{binding:1,resource:n},{binding:2,resource:c},{binding:3,resource:{buffer:this.statsBuf}}]})}this.reset()}reset(i=this.params.tFar){const t=this.n;this.params.tFar=i;const s=new Float32Array(t*t*4),e=this.params.alloyOn?this.params.c0:0;for(let r=0;r<t*t;r++)s[r*4+1]=i,s[r*4+2]=e;for(const r of this.stateTex)this.device.queue.writeTexture({texture:r},s,{bytesPerRow:t*16},[t,t]);const a=new Uint32Array(t*t);for(const r of this.grainTex)this.device.queue.writeTexture({texture:r},a,{bytesPerRow:t*4},[t,t]);this.dir=0,this.frame=0,this.simTime=0,this.nextId=1,this.frontX=1,this.pendingSeeds=[],this.theta0CPU.fill(0),this.device.queue.writeBuffer(this.theta0Buf,0,this.theta0CPU),this.device.queue.writeBuffer(this.twinCtrBuf,0,new Uint32Array([d-1]))}resetMold(i,t=1.15,s=.06){const e=this.n,a=new Float32Array(e*e*4),r=new Uint32Array(e*e);for(let n=0;n<e*e;n++){const c=i[n]===1;a[n*4]=c?0:1,a[n*4+1]=c?t:s,a[n*4+3]=c?0:-1}for(let n=0;n<e*e;n++)i[n]!==1&&(r[n]=1);for(const n of this.stateTex)this.device.queue.writeTexture({texture:n},a,{bytesPerRow:e*16},[e,e]);for(const n of this.grainTex)this.device.queue.writeTexture({texture:n},r,{bytesPerRow:e*4},[e,e]);this.dir=0,this.frame=0,this.simTime=0,this.frontX=1,this.pendingSeeds=[],this.theta0CPU.fill(0),this.theta0CPU[1]=Math.random()*(2*Math.PI/this.params.aniMode),this.nextId=2,this.device.queue.writeBuffer(this.theta0Buf,0,this.theta0CPU),this.device.queue.writeBuffer(this.twinCtrBuf,0,new Uint32Array([d-1]))}addSeed(i,t,s=4,e,a=2){let r=this.nextId++;r>=d&&(this.nextId=2,r=1);const n=e??Math.random()*(2*Math.PI/this.params.aniMode);return this.theta0CPU[r]=n,this.device.queue.writeBuffer(this.theta0Buf,r*4,this.theta0CPU,r,1),this.pendingSeeds.push({x:i,y:t,r:s,id:r,tact:a}),r}quench(i=.25){this.pendingQuench+=i}addTwinSeed(i,t,s=4){const e=Math.random()*(2*Math.PI/this.params.aniMode),a=Math.random()*Math.PI*2,r=s*.45;this.addSeed(i-Math.cos(a)*r,t-Math.sin(a)*r,s,e),this.addSeed(i+Math.cos(a)*r,t+Math.sin(a)*r,s,e+Math.PI/this.params.aniMode)}chillWall(i="bottom",t=42){const s=this.n;for(let e=0;e<t;e++){const a=((e+.5)/t+(Math.random()-.5)*.6/t)*s;i==="bottom"?this.addSeed(a,s-3,3.5):this.addSeed(3,a,3.5)}}writeParams(i){const t=this.params,s=new Uint32Array(this.paramData),e=new Float32Array(this.paramData);s[0]=this.n,s[1]=this.frame,e[2]=t.dx,e[3]=t.dt,e[4]=t.epsBar,e[5]=t.delta,e[6]=t.aniMode,e[7]=t.tau,e[8]=t.alpha,e[9]=t.gamma,e[10]=t.latent,e[11]=t.noiseAmp,e[12]=t.tFar,e[13]=t.coolRate,e[14]=t.heatIn,s[15]=i,e[16]=this.simTime,s[17]=t.scen,e[18]=t.gradG,e[19]=this.frontX,e[20]=t.weldX,e[21]=t.weldY,e[22]=t.weldPow,e[23]=t.weldSig,s[24]=t.alloyOn,e[25]=t.c0,e[26]=t.mLiq,e[27]=t.kPart,e[28]=t.dSol,e[29]=this.pendingQuench,e[30]=t.twinProb,s[31]=this.nextId,s[32]=this.probe?Math.round(this.probe.x):4294967295,s[33]=this.probe?Math.round(this.probe.y):4294967295,e[34]=t.holdT,e[35]=t.holdRate,this.device.queue.writeBuffer(this.paramBuf,0,this.paramData)}dispatch(i){const t=Math.ceil(this.n/8);i.dispatchWorkgroups(t,t)}step(i){if(this.busy)return 0;const t=this.device,s=Math.max(1,Math.floor(16e7/(this.n*this.n))),e=Math.min(i,s);let a=0;if(this.pendingSeeds.length>0){const u=this.pendingSeeds.splice(0,R),h=new Float32Array(u.length*P);u.forEach((p,b)=>{const f=b*P;h[f]=p.x,h[f+1]=p.y,h[f+2]=p.r,h[f+3]=p.id,h[f+4]=p.tact}),t.queue.writeBuffer(this.seedBuf,0,h),a=u.length}const r=a>0||this.pendingQuench!==0;if(e===0&&!r)return 0;this.frame++,this.params.scen===1&&(this.frontX=1+this.params.pullV*this.simTime),this.writeParams(a),this.pendingQuench=0;const n=t.createCommandEncoder(),c=n.beginComputePass();let l=this.dir;r&&(c.setPipeline(this.stampPipe),c.setBindGroup(0,this.stampBG[l]),this.dispatch(c),l=1-l);for(let u=0;u<e;u++)c.setPipeline(this.fluxPipe),c.setBindGroup(0,this.fluxBG[l]),this.dispatch(c),c.setPipeline(this.updatePipe),c.setBindGroup(0,this.updateBG[l]),this.dispatch(c),l=1-l;return c.end(),t.queue.submit([n.finish()]),this.dir=l,this.simTime+=e*this.params.dt,this.inFlight++,t.queue.onSubmittedWorkDone().then(()=>{this.inFlight=Math.max(0,this.inFlight-1)}),e}async readStats(){if(this.statsInFlight)return null;this.statsInFlight=!0;const i=this.device;this.writeParams(0);const t=i.createCommandEncoder();t.clearBuffer(this.statsBuf);const s=t.beginComputePass();s.setPipeline(this.statsPipe),s.setBindGroup(0,this.statsBG[this.dir]),this.dispatch(s),s.end(),t.copyBufferToBuffer(this.statsBuf,0,this.statsStaging,0,this.statsBuf.size),t.copyBufferToBuffer(this.theta0Buf,0,this.statsStaging,this.statsBuf.size,d*4),i.queue.submit([t.finish()]);try{await this.statsStaging.mapAsync(GPUMapMode.READ)}catch{return this.statsInFlight=!1,null}const e=this.statsStaging.getMappedRange().slice(0),a=new Uint32Array(e,0,8+d),r=new Float32Array(e,(8+d)*4,d);this.statsStaging.unmap(),this.statsInFlight=!1;const n=this.n*this.n,c=a[0],l=a[1],u=l>0?a[2]/1e3/l:0,h=this.probe?a[4]/1e3-1:null,p=this.probe?a[5]/1e3:null,b=Math.max(20,this.n*this.n*1e-5),f=I*1e3/this.n,m=[],v=2*Math.PI/this.params.aniMode,w=new Array(18).fill(0);let y=0;for(let x=1;x<d;x++){const S=a[8+x];if(S>b){y+=S,m.push(2*Math.sqrt(S/Math.PI)*f);const L=(r[x]%v+v)%v;w[Math.min(17,Math.floor(L/v*18))]+=S}}const T=m.length,M=T>0?y/T:0;let C=null;if(T>=3&&M>0){const x=M*(f/1e3)**2;C=3.322*Math.log10(1/x)-2.954}return{fracSolid:c/n,grainCount:T,meanAreaPx:M,astm:C,interfaceT:u,diamsUm:m,probeT:h,probePhi:p,oriRose:w}}async readLine(i,t,s,e,a=400){const r=this.n,n=m=>Math.min(r-1,Math.max(0,m));i=n(i),t=n(t),s=n(s),e=n(e);const c=Math.floor(Math.min(t,e)),l=Math.ceil(Math.max(t,e))-c+1,u=r*16,h=this.device.createBuffer({size:u*l,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),p=this.device.createCommandEncoder();p.copyTextureToBuffer({texture:this.stateTex[this.dir],origin:{x:0,y:c}},{buffer:h,bytesPerRow:u},{width:r,height:l}),this.device.queue.submit([p.finish()]);try{await h.mapAsync(GPUMapMode.READ)}catch{return h.destroy(),null}const b=new Float32Array(h.getMappedRange().slice(0));h.unmap(),h.destroy();const f=new Float32Array(a);for(let m=0;m<a;m++){const v=m/(a-1),w=Math.round(i+(s-i)*v),y=Math.round(t+(e-t)*v);f[m]=b[((y-c)*r+w)*4]}return f}}const D={generic:{label:"model metal (pure)",note:"Kobayashi's dimensionless pure metal — the reference crystal every phase-field paper grows first.",params:{aniMode:4,delta:.04,latent:1.6,alloyOn:0,meltGlow:1}},al:{label:"aluminum · Al–Cu",note:"FCC, ⟨100⟩ arms. Freezes at 660 °C, so the melt only glows dull red. Al castings grow feathery twinned grains — try the twin rate slider.",params:{aniMode:4,delta:.045,latent:1.35,alloyOn:1,c0:.3,mLiq:.5,kPart:.14,dSol:.9,meltGlow:.55}},steel:{label:"steel · Fe–C",note:"BCC δ-ferrite, 4-fold ⟨100⟩ dendrites. Pours at ~1540 °C — white-hot, the brightest melt here.",params:{aniMode:4,delta:.03,latent:1.8,alloyOn:1,c0:.35,mLiq:.5,kPart:.3,dSol:.7,meltGlow:1}},ni:{label:"nickel superalloy",note:"Turbine-blade metal, grown as one single crystal in a Bridgman furnace — try the BRIDGMAN scenario.",params:{aniMode:4,delta:.04,latent:1.7,alloyOn:1,c0:.3,mLiq:.4,kPart:.35,dSol:.6,meltGlow:.95}},co:{label:"cobalt alloy",note:"Surprise: Co freezes FCC, so its dendrites are 4-fold like steel's. It only turns HCP at 417 °C, long after solidifying.",params:{aniMode:4,delta:.035,latent:1.7,alloyOn:1,c0:.3,mLiq:.45,kPart:.25,dSol:.7,meltGlow:.95}},cu:{label:"copper · bronze",note:"The oldest cast metal — bronze bells, brass fittings. FCC, freezes at 1085 °C with an honest orange glow.",params:{aniMode:4,delta:.04,latent:1.6,alloyOn:1,c0:.3,mLiq:.45,kPart:.2,dSol:.8,meltGlow:.8}},mg:{label:"magnesium · AZ91",note:"HCP — a metal that grows genuine 6-fold dendrites, snowflakes in magnesium.",params:{aniMode:6,delta:.04,latent:1.5,alloyOn:1,c0:.35,mLiq:.5,kPart:.35,dSol:.8,meltGlow:.6}},zn:{label:"zinc · spangle",note:"The spangle on galvanized steel is exactly this: HCP 6-fold crystals. At 420 °C the melt does not glow at all — just liquid silver.",params:{aniMode:6,delta:.045,latent:1.6,alloyOn:0,meltGlow:.3}},ice:{label:"water · ice",note:"Hexagonal ice — the one fact behind every 6-armed snowflake. Twinned seeds grow the rare 12-branched flake.",params:{aniMode:6,delta:.04,latent:1.8,noiseAmp:.014,alloyOn:0,meltGlow:.12}},scn:{label:"succinonitrile (SCN)",note:"NASA's transparent model metal — flown on the Space Shuttle to film dendrites growing. Weak anisotropy, soft rounded tips.",params:{aniMode:4,delta:.012,latent:1.4,noiseAmp:.016,alloyOn:0,meltGlow:.18}}};class W{constructor(i,t,s){o(this,"device");o(this,"ctx");o(this,"canvas");o(this,"pipe");o(this,"rbuf");o(this,"bg",[]);o(this,"rdata",new ArrayBuffer(64));o(this,"zoom",1);o(this,"cx",.5);o(this,"cy",.5);o(this,"pixelSize",0);o(this,"paletteOn",!1);o(this,"stainMode",0);o(this,"ebsdOn",!1);this.device=i,this.canvas=t,this.ctx=t.getContext("webgpu");const e=navigator.gpu.getPreferredCanvasFormat();this.ctx.configure({device:i,format:e,alphaMode:"opaque"}),this.pipe=i.createRenderPipeline({layout:"auto",vertex:{module:i.createShaderModule({code:G}),entryPoint:"vmain"},fragment:{module:i.createShaderModule({code:G}),entryPoint:"fmain",targets:[{format:e}]},primitive:{topology:"triangle-list"}}),this.rbuf=i.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.rebind(s)}rebind(i){for(const t of[0,1])this.bg[t]=this.device.createBindGroup({layout:this.pipe.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.rbuf}},{binding:1,resource:i.stateTexture(t).createView()},{binding:2,resource:i.grainTexture(t).createView()},{binding:3,resource:{buffer:i.theta0Buffer}}]})}resize(){const i=Math.min(window.devicePixelRatio||1,2),t=Math.floor(this.canvas.clientWidth*i),s=Math.floor(this.canvas.clientHeight*i);(this.canvas.width!==t||this.canvas.height!==s)&&(this.canvas.width=t,this.canvas.height=s)}clampView(){this.zoom=Math.min(24,Math.max(1,this.zoom));const i=.5/this.zoom;this.cx=Math.min(1-i,Math.max(i,this.cx)),this.cy=Math.min(1-i,Math.max(i,this.cy))}resetView(){this.zoom=1,this.cx=.5,this.cy=.5}clientToCover(i,t){const s=this.canvas.getBoundingClientRect(),e=this.canvas.width/s.width,a=(i-s.left)*e,r=(t-s.top)*e,n=Math.max(this.canvas.width,this.canvas.height),c=(this.canvas.width-n)*.5,l=(this.canvas.height-n)*.5;return{x:(a-c)/n,y:(r-l)/n}}clientToGrid(i,t,s){const e=this.clientToCover(i,t),a=((e.x-.5)/this.zoom+this.cx)*s,r=((e.y-.5)/this.zoom+this.cy)*s;return a<0||r<0||a>=s||r>=s?null:{x:a,y:r}}zoomAt(i,t,s){const e=this.clientToCover(i,t),a=(e.x-.5)/this.zoom+this.cx,r=(e.y-.5)/this.zoom+this.cy;this.zoom*=s,this.zoom=Math.min(24,Math.max(1,this.zoom)),this.cx=a-(e.x-.5)/this.zoom,this.cy=r-(e.y-.5)/this.zoom,this.clampView()}panBy(i,t){const s=this.canvas.getBoundingClientRect(),e=this.canvas.width/s.width,a=Math.max(this.canvas.width,this.canvas.height);this.cx-=i*e/a/this.zoom,this.cy-=t*e/a/this.zoom,this.clampView()}gridToClient(i,t,s){const e=this.canvas.getBoundingClientRect(),a=this.canvas.width/Math.max(e.width,1),r=Math.max(this.canvas.width,this.canvas.height),n=(this.canvas.width-r)*.5,c=(this.canvas.height-r)*.5,l=(i/s-this.cx)*this.zoom+.5,u=(t/s-this.cy)*this.zoom+.5;return{x:(l*r+n)/a+e.left,y:(u*r+c)/a+e.top}}cssPxPerCell(i){const t=this.canvas.getBoundingClientRect();return Math.max(t.width,t.height)/i*this.zoom}render(i,t,s){this.resize();const e=new Uint32Array(this.rdata),a=new Float32Array(this.rdata);e[0]=t,e[1]=i.n,a[2]=this.canvas.width,a[3]=this.canvas.height,a[4]=s,a[5]=i.params.aniMode,a[6]=i.params.tFar,a[7]=this.zoom,a[8]=this.cx,a[9]=this.cy,a[10]=this.pixelSize,e[11]=this.paletteOn?1:0,e[12]=i.params.alloyOn,a[13]=i.params.c0,a[14]=i.params.meltGlow,e[15]=this.stainMode&255|(this.ebsdOn?256:0),this.device.queue.writeBuffer(this.rbuf,0,this.rdata);const r=this.device.createCommandEncoder(),n=r.beginRenderPass({colorAttachments:[{view:this.ctx.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:.04,g:.04,b:.05,a:1},storeOp:"store"}]});n.setPipeline(this.pipe),n.setBindGroup(0,this.bg[i.dir]),n.draw(3),n.end(),this.device.queue.submit([r.finish()])}}export{I as D,k as L,D as M,W as R,F as S};
